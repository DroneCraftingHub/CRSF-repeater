...
void CRSF::sendChannels(uint16_t channels[CRSF_MAX_CHANNEL]) {
    preparePacket(crsfPacket, channels);
    m_port->write(crsfPacket, CRSF_PACKET_SIZE);
}


void CRSF::preparePacket(uint8_t packet[], uint16_t channels[CRSF_MAX_CHANNEL]) {
   ...
   /*    
    ...
    packet[0] = CRSF_ADDRESS_MODULE; //Header
